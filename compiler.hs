...
program (currentToken, rest, cline) table
  | (tokenType currentToken) `elem` (predict "program_data") =
    ...
    (ti2, (programDecls table1 "global") ++ "mem[top] = base;\nmem[top + 1]=(long)&&exit;\nmem[top + 3] = top;\ntop = top + 4;\ngoto mainFunc;\nexit:;\nexit(0);\n" ++ code1, table1)
...
global_decl_tail (currentToken, rest, cline) table typeNameCode idenValue
  ...
  | (tokenType currentToken) == "(" =
    ...
    let table1 = (Table lc (Temps (Counts c1 t1, Counts 0 [], Counts 0 []))) in
    let (ti2, code1, table2) = (parameter_list ti1 table1) in
    let (currentToken3, rest3, cline3) = (match ti2 ")") in
    let (ti4, code2, table3) = (func_tail (currentToken3, rest3, cline3) table2) in
    if (currentToken3 == ";") then (ti4, code2, table3) else (ti4, idenValue ++ "Func:;\n" ++ code2, table3)
  | otherwise = errorWithoutStackTrace ("Syntax Error: line " ++ (show cline) ++ ": unexpected token " ++ "\"" ++ currentToken ++ "\"")
...
func_tail (currentToken, rest, cline) table
  ...
  | (tokenType currentToken) == "{" =
    ...
    (ti4, (programDecls table2 "local") ++ code1 ++ "top = base;\ngoto *mem[base-3];\n", table2)
...
assignment_or_general_func_call_tail (currentToken, rest, cline) table fn idenValue
  | (tokenType currentToken) == "=" =
    ...
    (ti3, named_var ++ " = " ++ code1 ++ ";\n", table1, fn1)
  | (tokenType currentToken) == "(" = case idenValue of
      "read" -> let ti1 = (match (currentToken, rest, cline) "(") in
                let (ti2, code1, table1, fn1) = (expression ti1 table fn) in--let (ti2, code1, table1, fn1) = (expr_list ti1 table fn) in
                let ti3 = (match ti2 ")") in
                let ti4 = (match ti3 ";") in
                (ti4, idenValue ++ "(" ++ code1 ++ ");\n", table1, fn1)
      "write" ->let ti1 = (match (currentToken, rest, cline) "(") in
                let (ti2, code1, table1, fn1) = (expression ti1 table fn) in--let (ti2, code1, table1, fn1) = (expr_list ti1 table fn) in
                let ti3 = (match ti2 ")") in
                let ti4 = (match ti3 ";") in
                (ti4, idenValue ++ "(" ++ code1 ++ ");\n", table1, fn1)
      otherwise -> let ti1 = (match (currentToken, rest, cline) "(") in
                   let (ti2, code1, table1, fn1) = (expr_list ti1 table fn) in
                   let ti3 = (match ti2 ")") in
                   let ti4 = (match ti3 ";") in
                   let ((Table lc (Temps (Counts c1 t1, Counts c2 t2, Counts c3 t3))), l1) = (getLabel table1) in
                   (ti4, code1 ++ "mem[top] = base;\nmem[top + 1] = (long)&&" ++ l1 ++ ";\nmem[top + 3] = top;\ntop = top + 4;\ngoto " ++ idenValue ++ "Func;\n" ++ l1 ++ ":;\nbase = mem[top - 4];\ntop = mem[top]" ++ ";\n", (Table lc (Temps (Counts c1 t1, Counts c2 t2, Counts c3 t3))), "")
  | otherwise = errorWithoutStackTrace ("Syntax Error: line " ++ (show cline) ++ ": unexpected token " ++ "\"" ++ currentToken ++ "\". Expecting \"=\" or \"(\"")
...
expr_list (currentToken, rest, cline) table fn
  ...
  | (tokenType currentToken) `elem` (predict "non_empty_expr_list") =
    let (ti1, code1, table1, fn1) = (non_empty_expr_list (currentToken, rest, cline) table fn 0) in
    (ti1, code1, table1, fn1)
...
non_empty_expr_list :: TokenIterator -> Table -> String -> Int -> (TokenIterator, String, Table, String)
non_empty_expr_list (currentToken, rest, cline) table fn paramCount
  | (tokenType currentToken) `elem` (predict "expression") =
    let (ti1, code1, table1, fn1) = (expression (currentToken, rest, cline) table fn) in
    let (ti2, code2, table2, fn2) = (non_empty_expr_list_tail ti1 table1 fn1 (paramCount + 1)) in
    (ti2, "mem[top + " ++ (show paramCount) ++ "] = " ++ code1 ++ ";\n" ++ code2, table2, fn2)
...
non_empty_expr_list_tail :: TokenIterator -> Table -> String -> Int -> (TokenIterator, String, Table, String)
non_empty_expr_list_tail (currentToken, rest, cline) table fn paramCount
  | (tokenType currentToken) == "," =
    let ti1 = (match (currentToken, rest, cline) ",") in
    let (ti2, code1, table1, fn1) = (expression ti1 table fn) in
    let (ti3, code2, table2, fn2) = (non_empty_expr_list_tail ti2 table1 fn1 (paramCount + 1)) in
    (ti3, "mem[top + " ++ (show paramCount) ++ "] = " ++ code1 ++ ";\n" ++ code2, table2, fn2)
  | (tokenType currentToken) `elem` (parseTable "non_empty_expr_list_tail" "FOLLOW") =
    ((currentToken, rest, cline), "top = top + " ++ (show paramCount) ++ ";\n", table, fn)
  | otherwise = errorWithoutStackTrace ("Syntax Error: invalid expression at line " ++ (show cline) ++ ": unexpected token " ++ "\"" ++ currentToken ++ "\"")
...
factor_tail (currentToken, rest, cline) table fn idenValue
  ...
  | (tokenType currentToken) == "(" =
    ...
    let (ti2, code1, table1, fn1) = (expr_list ti1 table fn) in
    let ti3 = (match ti2 ")") in
    let ((Table lc (Temps (Counts c1 t1, Counts c2 t2, Counts c3 t3))), named_var) = (addTemp table1) in
    let (table3, l1) = getLabel (Table lc (Temps (Counts c1 t1, Counts c2 t2, Counts c3 t3))) in
    let fn2 = (fn1 ++ code1 ++ "mem[top] = base;\nmem[top + 1] = (long)&&" ++ l1 ++ ";\nmem[top + 3] = top;\ntop = top + 4;\ngoto " ++ idenValue ++ "Func;\n" ++ l1 ++ ":;\n" ++ "base = mem[top - 4];\n" ++ "top = mem[top - 1];\n" ++ named_var ++ " = mem[top + 2];\n") in
...
-- Init Symbol table
  let symbolTable = Table 1 (Temps (Counts 0 [], Counts 0 [], Counts 0 []))
-- Start parsing
